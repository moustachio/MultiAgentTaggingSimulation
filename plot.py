...
truDist = [int(line.strip().split('\t')[1]) for line in open('/Users/AxelStreichen/Dropbox/Research/PROJECTS/Tagging/OLD/crawlerAnalysisV2/data/generalData/annotationsPerTag')]
#truDist = [int(line.strip().split('\t')[1]) for line in open('C:/Users/jlorince/Dropbox/Research/PROJECTS/Tagging/OLD/crawlerAnalysisV2/data/generalData/annotationsPerTag')]
null = [44.34860660399451]*747275
allPtop = [paramKey[i]['pTop'] for i in paramKey]
    ...
    for t in paramKey:
        if paramKey[t]['pTop'] == pTop:
            ...
            if pTop=='0.9':
                ...
                lab = 'Basic Model'
            ax.plot(x,label=lab)
ax.plot(truDist, label='Empirical Dist.')
ax.plot(null,label='Null Model')
